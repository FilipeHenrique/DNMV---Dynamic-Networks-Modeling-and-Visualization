...
def disparity_filter(G, weight='weight'):
    ...
    if nx.is_directed(G): #directed case    
        ...
        for u in G:
            ...
            successorsList = list(G.successors(u))
            
            if k_out > 1:
                sum_w_out = sum(np.absolute(G[u][v][weight]) for v in successorsList)
                for v in successorsList:
                    w = G[u][v][weight]
                    p_ij_out = float(np.absolute(w))/sum_w_out
                    alpha_ij_out = 1 - (k_out-1) * integrate.quad(lambda x: (1-x)**(k_out-2), 0, p_ij_out)[0]
                    N.add_edge(u, v, weight = w, alpha_out=float('%.4f' % alpha_ij_out))
                    
            elif k_out == 1 and G.in_degree(successorsList[0]) == 1:
...
def disparity_filter_alpha_cut(G,alpha_t,weight='weight', cut_mode='or'):
